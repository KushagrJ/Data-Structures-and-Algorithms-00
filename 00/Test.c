 ...
 * An instance of a problem consists of the input (satisfying whatever
 ...
 * An algorithm for a computational problem is correct if, for every problem
   instance provided as input, it halts, i.e. finishes its computing in finite
   time, and outputs the correct solution to the problem instance.
 ...
 * Because many programs use it as an intermediate step, sorting is a
   ...
   As a result, there are a large number of good sorting algorithms available.
 * Which sorting algorithm is the best for a given application depends on the
   number of items to be sorted, the extent to which the items are already
